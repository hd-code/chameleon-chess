import { FieldColor, getFieldColor, isFieldColor } from "./Board";
import { Color } from "./Color";
import { isWithinLimits, Limits } from "./Limits";
import { isPlayerColor, PlayerColor } from "./Players";
import { isPosition, isSamePosition, Position } from "./Position";
import { getRole, getRoles, Role, Roles } from "./Roles";
...
export function isPawn(pawn: unknown): pawn is Pawn {
    return (
        ...
        hasKey(pawn, "player", isPlayerColor) &&
...
export function createPawns(playerColor: PlayerColor): Pawn[] {
    return initialPawns[playerColor].map(([row, col, knightColor]) => ({
        knightColor,
        playerColor,
        position: { row, col },
    }));
...
const initialPawns: { [color in PlayerColor]: [number, number, FieldColor][] } =
    {
        [Color.red]: [
            [7, 0, Color.red],
            [7, 1, Color.green],
...
function getMoveType(
    ...
    if (pawnOnField.playerColor !== pawnToMove.playerColor) {

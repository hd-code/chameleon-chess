...
describe("GameState", () => {
    ...
    describe(createGameState.name, () => {
        [
            [[true, true, true, true], 16, true],
            [[true, true, false, true], 12, true],
            [[true, false, true, false], 8, true],
            [[true, false, false, false], 4, false],
            [[false, true, false, false], 4, false],
        ].forEach(([a, numOfPawns, ok]) => {
            it(`${a} => ${ok}`, () => {
                const gs = createGameState(
                    ...(a as [boolean, boolean, boolean, boolean]),
                );
                assert.strictEqual(gs != null, ok);
                if (gs != null) {
                    assert.strictEqual(gs.pawns.length, numOfPawns);
                }
            });
        });
    });
    ...
    describe(getNextGameStates.name, () => {
        ...
        it("all game states should have player blue on turn", () => {
            nextGSs.forEach((gs) => assert.strictEqual(gs.player, Color.blue));
        });
    });
});

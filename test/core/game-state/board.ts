...
import {
    ...
    isSamePosition,
    sortPositions,
} from 'core/game/game-state/board';
...
describe('core/game-state/board', () => {
    ...
    describe(getBoard.name, () => {
        ...
        it('should have 8 columns', () => board.forEach(row => expect(row.length).toBe(8)));
        it('should only contain integers from 0 to 7', () => {
            const regex = /[0-7]/;
            board.forEach(row => row.forEach(val => expect(regex.test(val.toString())).toBeTruthy()));
        });
    });
